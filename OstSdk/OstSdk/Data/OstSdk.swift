...
class OstSdk {
    ...
    static func getUser(_ id: String) throws -> OSTUser? {
        return try OSTUserModelRepository.sharedUser.get(id)
    }
    
    static func initUser(_ userJson: [String: Any], success: ((OSTUser?) -> Void)?, failure: ((Error) -> Void)?) {
        return OSTUserModelRepository.sharedUser.save(userJson, success: success, failure: failure)
    }
    
    static func getRule(_ id: String) throws -> OSTRule? {
        return try OSTRuleModelRepository.sharedRule.get(id)
    }
    
    static func initRule(_ ruleJson: [String: Any], success: ((OSTRule?) -> Void)?, failure: ((Error) -> Void)?) {
        return OSTRuleModelRepository.sharedRule.save(ruleJson, success: success, failure: failure)
    }
    
    static func getTokenHolder(_ id: String) throws -> OSTTokenHolder? {
        return try OSTTokenHolderRepository.sharedTokenHolder.get(id)
    }
    
    static func initTokenHolder(_ tokenHolderJson: [String: Any], success: ((OSTTokenHolder?) -> Void)?, failure: ((Error) -> Void)?) {
